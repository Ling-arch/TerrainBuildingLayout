...
namespace voronoi2
{
    ...
    bool Cell::is_inside(const Vector2d &p) const{
        double wn = util::winding_number(vtx2xy, p);
        return std::abs(wn - 1.0) < 0.1;
    }

    double Cell::area() const{
        return util::polygon_area(vtx2xy);
    }
    ...
    vector<Cell> voronoi_cells(
        ...
        vector<Vector2d> vtxl2xy = util::to_vec2_array(vtxl2xy_flat);
        vector<Vector2d> site2xy = util::to_vec2_array(site2xy_flat);
    ...
    Vector2d position_of_voronoi_vertex(
        ...
        if (info[1] == std::numeric_limits<size_t>::max()){
            ...
            return util::line_intersection(l1, (l2 - l1), mid, bisdir);
        }
        else{
            ...
            return util::circumcenter(s0, s1, s2);
        }
    }
}
